...
class TokensControllerTest < ActionController::TestCase
  ...
  test "should create token" do
    assert_difference('Token.count') do
      post :create, token: { name: @token.name, number_of_requests: @token.number_of_requests, status: @token.status, token: @token.token }
    end
  ...
  test "should update token" do
    patch :update, id: @token, token: { name: @token.name, number_of_requests: @token.number_of_requests, status: @token.status, token: @token.token }
    assert_redirected_to token_path(assigns(:token))
  end
